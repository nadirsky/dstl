...
# prefix = "/home/maksymilian/Desktop/Part/"
...
#     channel_type=neptune.ChannelType.NUMERIC)
#
# epoch_train_val_loss_channel = ctx.job.create_channel(
#     name='epoch_train_val_loss',
#     channel_type=neptune.ChannelType.NUMERIC)
# epoch_train_val_acc_channel = ctx.job.create_channel(
#     name='epoch_train_val_acc',
#     channel_type=neptune.ChannelType.NUMERIC)
#
# lr_channel = ctx.job.create_channel(
#     name='lr',
#     channel_type=neptune.ChannelType.NUMERIC)
...
#         self.epoch_id += 1
#         epoch_train_val_loss_channel.send(x=self.epoch_id, y=float(logs.get('val_loss')))
#         epoch_train_val_acc_channel.send(x=self.epoch_id, y=float(logs.get('val_acc')))
...
# print(X_val)
# print(y_val)
# logs_channel.send(x=time.time(), y="X_val = " + str(X_val))
# logs_channel.send(x=time.time(), y="y_val = " + str(y_val))
#
# logs_channel.send(x=time.time(), y="X_val.shape = " + str(X_val.shape))
# logs_channel.send(x=time.time(), y="y_val.shape = " + str(y_val.shape))
...
# X_val = load_image(prefix + "validation/vehicles/24213.jpg")
# y_val = load_image(prefix + "validation_masks/vehicles/24213.jpg")

X_train_prepared = prepare_20_channels(X_train)
X_val_prepared = prepare_20_channels(X_val)

# logs_channel.send(x=time.time(), y="X_image = " + str(X_train_prepared))
# logs_channel.send(x=time.time(), y="X_image.shape = " + str(X_train_prepared.shape))
#
# logs_channel.send(x=time.time(), y="y_val.non_zero = " + str(np.count_nonzero(y_train)))

# X_val_prepared = X_val.reshape((1,) + X_val.shape)
# X_val_prepared = np.transpose(X_val_prepared, (0, 3, 1, 2))
...
class EpochEndCallback(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        if float(logs.get('val_loss')) > self.v_loss:
            self.v_loss = float(logs.get('val_loss'))
            action_save_model("max_val_loss")
...
path = "/mnt/ml-team/satellites/max.sokolowski/satellites/maksymilian.sokolowski@codilime.com/miscSatellites/201702241427/0/jobs/6931e590-e5c7-44de-981d-0c06443af5ab/src/model_5.h5"
